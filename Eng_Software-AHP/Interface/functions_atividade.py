...
def decimal():
    valores = []
    for t in range(len(Janela_2.criterio)):
        print("MATRIZ do ", t)
        matriz = janela_3.ui.tabWidget.findChild(QtCore.QObject, "tableWidget_" + str(t))
        for i in range(len(Janela_2.atividade)):
            for j in range(len(Janela_2.atividade)):
...
def vetor_eigen(normal):
    ...
    for t in range(len(Janela_2.criterio)):
        ...
        for i in range(len(Janela_2.atividade)):
            ...
            for j in range(len(Janela_2.atividade)):
                s += matriz[i + aux + aux2]
                aux += len(Janela_2.atividade)
            eigen.append(s / len(Janela_2.atividade))
        aux2 += len(Janela_2.atividade) * len(Janela_2.atividade)
...
def ci(lambd):
    ...
    for t in range(len(Janela_2.criterio)):
        print("MATRIZ do ", t)
        ci = (lambd_v[t] - len(Janela_2.atividade)) / (len(Janela_2.atividade) - 1)
